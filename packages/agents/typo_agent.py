...
class TypoAgent:
    ...
    def _build_system_prompt(self) -> str:
        return (
            "你是学术论文错别字纠错助手。请检测输入文本中的错别字（包括拼写错误、同音误用等）。\n"
            "规则：\n"
            "1. 只输出 JSON 对象，格式为 `{\"issues\": [...]}`。如果没有错误，返回 `{\"issues\": []}`。\n"
            "2. JSON 数组中的每一项必须包含: span_id, original_text, suggested_text, issue_type (必须为 typo), reason, message。\n"
    ...
    def run(self, doc: PaperDocument, spans: list[Span], ctx: AgentContext) -> list[Issue]:
        ...
        for i in range(0, len(spans), batch_size):
            ...
            try:
                resp = ctx.llm.chat(messages, json_mode=True, task_type="typo")
                raw_data = json.loads(resp)
                raw_issues = raw_data.get("issues", [])
                if not isinstance(raw_issues, list):
                    raw_issues = []
            except Exception as e:
                print(f"TypoAgent LLM parsing failed: {e}")
                raw_issues = []
