...
export function reviewStatusClass(status: string): string {
  ...
  if (status === "pending_release") return "badge badge-release";
  if (status.endsWith("failed")) return "badge badge-failed";
  if (status === "issues_found") return "badge badge-issues";
  if (status === "parsing" || status === "analyzing") return "badge badge-progress";
